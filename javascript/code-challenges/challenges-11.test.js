...
const totalSum = (input) => {
  ...
  input.forEach((element) => {
    let sum = element.reduce((sum, value) => {
      return sum + value;
    });
    sumArr += sum;
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  let arr = input.map((arr) => {
    let powerd = [];
    arr.forEach((num) => {
      if (typeof num == "number" && num % 5 == 0) powerd.push(2 ** num);
    });
    return powerd;
  });
  return arr;
...
let findMaleAndFemale = (data) => {
  let filterArr = data.filter((obj) => {
    return obj.gender === "male" || obj.gender === "female";
  });
  let names = filterArr.map((item) => {
    return item.name;
  });
...
let findShortest = (data) => {
  let shortest = data[0];
  data.forEach((chars) => {
    if (parseInt(chars.height) < parseInt(shortest.height)) shortest = chars;
  });
