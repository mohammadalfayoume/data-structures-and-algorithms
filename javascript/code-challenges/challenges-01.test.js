...
const speaker = (words, callback) => {
  ...
  words.forEach(element => {
    let temporary=callback(element)
...
const createList = (availableItems) => {
  ...
  availableItems.forEach(element => {
    if (element.available == true) {
      availableArr.push(element.name)
    }
  });
  return availableArr
};
...
const fizzbuzz = (arr) => {
  const newArr=[]
  arr.forEach(element => {
    (element %3==0 && element %5==0) ? newArr.push("Fizz Buzz")
    : (element %3==0) ? newArr.push("Fizz")
    : (element %5==0) ? newArr.push("Buzz")
    : newArr.push(element)
