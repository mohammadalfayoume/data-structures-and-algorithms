...
const validateEmail = (email) => {
  let find= /^(\w+).(\w+)(@)(\w+)(.com|.net|.org)$/ig
  return find.test(email)
};
...
const validatePhoneNumber = (phoneNumber) => {
  return /^((\d{10})|(\d{6}-\d{4})|(\d{3} \d{3} \d{4})|(\d{3}-\d{3}-\d{4})|(\d{3}-\d{3} \d{4})|(\d{3} \d{7})|(\(\d{3}\)\d{3} \d{4})|(\(\d{3}\) \d{3}-\d{4})|(\d{3} \d{3}-\d{4}))$/ig.test(phoneNumber)
};
