...
const sortByChildren = (charArray) => {
  return charArray.sort((a, b) => {
    if (a.children.length == b.children.length) {
      return a.name > b.name ? 1 : -1; // Add Bef Cqwe D E F
    } else {
...
const citiesAtoJ = (arr) => {
  let res1 = [];
  let regx1 = /\b[A-J](\w)*/; // Cleveland
  let regx2 = / \b[A-J](\w)*/; // Diego
  for (let i = 0; i < arr.length; i++) {
    if (regx1.test(arr[i]) && !regx2.test(arr[i])) res1.push(arr[i]);
  }
  return res1;
};
